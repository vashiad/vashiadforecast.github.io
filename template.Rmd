...
stateresults <- function(state){
  ...
  header <- htmltools::HTML(
      sprintf(
        '<div style="font-family:futura; font-size: 25px; width: 860px; float: left;">
...
<span style="font-size: 40px; color: #698dc5; float: left;"><strong>%s%%</strong></span> 
<span style="color: #f07763; font-size: 40px; float: right;"><strong>%s%%</strong></span><br clear="all" /> 
...
table <- function(state){
  ...
  df %>%
    ...
    column_spec(2, width = "2.65cm", bold=T)
}
...
statevoteshare <- function(state){
  ...
  config(displayModeBar = FALSE) %>%
  add_trace(y=~FinalPredictedBiden,name='Biden',mode='lines',line=list(color='rgb(105,141,197)',width=2.84),hoverinfo="y") %>%
  add_trace(y=~FinalPredictedTrump,name='Trump',mode='lines',line=list(color='rgb(240,119,99)',width=2.84),hoverinfo="y") %>%

...
```{r}
setwd("~/Desktop/vashiadforecast.github.io")
library(ggplot2)
library(tidyverse)
library(dplyr)
library(usmap)
...
BidenInput <- 50.0 #Input polling aggregate data here
TrumpInput <- 42.5 #Input polling aggregate data here
OtherInput <- 100 - (BidenInput + TrumpInput)
BidenNPVPollAvg <- BidenInput + (OtherInput * 0.35)
TrumpNPVPollAvg <- TrumpInput + (OtherInput * 0.35)
OtherNPVPollAvg <- OtherInput * 0.30
...
#Popular vote empty vectors
TrumpNPVWinVector <- rep(NA, nsims)
BidenNPVWinVector <- rep(NA, nsims)
OtherNPVWinVector <- rep(NA, nsims)
...
for (x in 1:nsims){
  ...
  TrumpNPVWinVector[x] <- weighted.mean(trumpstateresults[x,],data$EV.s)
  BidenNPVWinVector[x] <- weighted.mean(bidenstateresults[x,],data$EV.s)
  OtherNPVWinVector[x] <- weighted.mean(otherstateresults[x,],data$EV.s)
  if (TrumpVoteEV>=270){
...
write.csv(bidenstateresults,'bidenstateresults.csv',row.names = FALSE)
write.csv(trumpstateresults,'trumpstateresults.csv',row.names = FALSE)
write.csv(otherstateresults,'otherstateresults.csv',row.names = FALSE)
...
percentiles <- function(candidate){
  ...
  for (i in 1:ncol(candidate)){
    ...
    dataframe[i,2] <- quantile(candidate[,i],0.10)
    dataframe[i,3] <- mean(candidate[,i])
    dataframe[i,4] <- quantile(candidate[,i],0.90)
  }
  return(dataframe)
}
...
#Creating csv files for plots in the future (States)
library(dplyr)
statesupdatedcsv <- simulations %>%
  select("Name","FinalPredictedTrump","FinalPredictedBiden","FinalPredictedOther",
         "TrumpStateWinProb","BidenStateWinProb","OtherStateWinProb")
statesupdatedcsv <- cbind(marginpercentiles, statesupdatedcsv)
statesupdatedcsv <- cbind(Date=as.character(Sys.time()),statesupdatedcsv)
#write.csv(updatedcsv,'csvsample.csv',row.names = FALSE)
statesexistingcsv <- read.csv("statescsv.csv")
statesfinalcsv <- rbind(statesexistingcsv,statesupdatedcsv)
write.csv(statesfinalcsv,'statescsv.csv',row.names = FALSE)

#Creating csv file for plots in the future (National)
nationalupdatedcsv <- data.frame("Date" <- as.character(Sys.time()),
                                 "FinalPredictedTrump" <- round(mean(finalEV$TrumpEV),1),
                                "FinalPredictedBiden" <- round(mean(finalEV$BidenEV),1),
                                "FinalPredictedOther" <- round(mean(finalEV$OtherEV),1),
                                "TrumpStateWinProb" <- TProbability,
                                "BidenStateWinProb" <- BProbability,
                                "OtherStateWinProb" <- OProbability,
                                "TrumpNPV" <- round(mean(TrumpNPVWinVector),1),
                                "BidenNPV" <- round(mean(BidenNPVWinVector),1),
                                "OtherNPV" <- round(mean(OtherNPVWinVector),1),
                                "LowTrump" <- quantile(finalEV$TrumpEV,0.10),
                                "HighTrump" <- quantile(finalEV$TrumpEV,0.90),
                                "LowBiden" <- quantile(finalEV$BidenEV,0.10),
                                "HighBiden" <- quantile(finalEV$BidenEV,0.90),
                                "LowOther" <- quantile(finalEV$OtherEV,0.10),
                                "HighOther" <- quantile(finalEV$OtherEV,0.90))

colnames(nationalupdatedcsv) <- c("Date","FinalPredictedTrump","FinalPredictedBiden",
                                  "FinalPredictedOther","TrumpStateWinProb","BidenStateWinProb",  "OtherStateWinProb","TrumpNPV","BidenNPV","OtherNPV","LowTrump","HighTrump","LowBiden","HighBiden","LowOther","HighOther")

nationalexistingcsv <- read.csv("nationalcsv.csv")
nationalfinalcsv <- rbind(nationalexistingcsv,nationalupdatedcsv)
write.csv(nationalfinalcsv,'nationalcsv.csv',row.names = FALSE)
...
#Chance Map
chancemap <- plot_usmap(data = simulations2, values = "TrumpStateWinProb", color = "black",size=0.55) + 
...
htmltools::HTML('<div style="font-family:futura; font-size: 17px; width: 900px; float: left;" id="Header"><br>This is an alternative method to look at the election map.<br>Each state is visualized by the number of electoral votes it has. Hover over each state to view state by state chances. 
</div>')
...
df <- data.frame("Candidate" <- c("Joe Biden","Donald Trump","Others"),
           "Electoral votes" <- c(round(mean(finalEV$BidenEV),1),round(mean(finalEV$TrumpEV),1),round(mean(finalEV$OtherEV),1)),
           "Popular vote" <-  c(round(mean(BidenNPVWinVector),1),round(mean(TrumpNPVWinVector),1),round(mean(OtherNPVWinVector),1)))
...
nationalproj <- read.csv("nationalcsv.csv")
nationalproj$Date <- lubridate::as_datetime(nationalproj$Date)


evproj <- nationalproj %>%
  select("Date","FinalPredictedBiden","FinalPredictedTrump","FinalPredictedOther")

plot_ly(evproj,x=evproj$Date) %>%
  config(displayModeBar = FALSE) %>%
  add_trace(y=~FinalPredictedBiden,name='Biden',mode='lines',line=list(color='rgb(105,141,197)',width=2.84)) %>%
  add_trace(y=~FinalPredictedTrump,name='Trump',mode='lines',line=list(color='rgb(240,119,99)',width=2.84)) %>%
  add_trace(y=~FinalPredictedOther,name='Others',mode='lines',line=list(color='rgb(207,152,0)',width=2.84)) %>%
  layout(margin = list(t=40),title = 'Electoral votes over time',
         ...
         xaxis = list(fixedrange=TRUE,title = 'Date',
                      ...
                      spikethickness=2,
                      range = (c("2020-07-26 00:38:14","2020-07-30 00:38:14"))),
...
evproj2 <- nationalproj %>%
  select("Date","BidenStateWinProb","TrumpStateWinProb","OtherStateWinProb")

plot_ly(evproj2,x=evproj2$Date) %>%
  config(displayModeBar = FALSE) %>%
...
evproj2 <- nationalproj %>%
  select("Date","BidenNPV","TrumpNPV","OtherNPV")

plot_ly(evproj2,x=evproj2$Date) %>%
  config(displayModeBar = FALSE) %>%
  add_trace(y=~BidenNPV,name='Biden',mode='lines',line=list(color='rgb(105,141,197)',width=2.84)) %>%
  add_trace(y=~TrumpNPV,name='Trump',mode='lines',line=list(color='rgb(240,119,99)',width=2.84)) %>%
  add_trace(y=~OtherNPV,name='Others',mode='lines',line=list(color='rgb(207,152,0)',width=2.84)) %>%
...
p1 <- p0 + geom_vline(xintercept = 0, color = "gray30") +
  geom_point(shape=16,size = 2,color=ifelse(marginpercentiles$V3<0,"#698DC5","#F07763")) +

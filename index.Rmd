...
nationalupdatedcsv <- data.frame("Date" <- as.character(Sys.time()),
                                ...
                                "TrumpWinProb" <- TProbability,
                                "BidenWinProb" <- BProbability,
                                "OtherWinProb" <- OProbability,
...
colnames(nationalupdatedcsv) <- c("Date","FinalPredictedTrump","FinalPredictedBiden",
                                  "FinalPredictedOther","TrumpStateWinProb","BidenStateWinProb",
                                  "OtherStateWinProb","LowTrump","HighTrump","LowBiden","HighBiden",
                                  "LowOther","HighOther")
...
htmltools::HTML('<div style="font-family:futura; font-size: 14px; width: 900px; float: left;" id="Header"><br>Hover over each state to view state by state chances. 
</div>')
...
htmltools::HTML('<div style="font-family:futura; font-size: 14px; width: 900px; float: left;" id="Header"><br>This is an alternative method to look at the election map.<br>Each state is visualized by the number of electoral votes it has. Hover over each state to view state by state chances. 
</div>')
...
df <- data.frame("Candidate" <- c("Joe Biden","Donald Trump","Others"),
           ...
           "Popular vote" <-  c(round(BidenNPV2,1),round(TrumpNPV2,1),round(OtherNPV2,1)))
...
plot_ly(evproj,x=evproj$Date) %>%
  ...
  layout(margin = list(t=40),title = 'Electoral votes',
         font=list(family='Futura', size=12.8),
         xaxis = list(title = 'Date',
                      zeroline = TRUE),
         yaxis = list(title = 'Average electoral votes'),
         hovermode = "x unified",
...
plot_ly(evproj2,x=evproj2$Date) %>%
  ...
  layout(margin = list(t=40),title = 'Chances of winning Electoral college',
         font=list(family='Futura', size=12.8),
         xaxis = list(title = 'Date',
                      zeroline = TRUE),
...
htmltools::HTML('<div style="font-family:futura; font-size: 30px; width: 900px; float: left;" id="Header">
<strong>A Tale of 20,000 Individual Simulations<br> <span style="font-size: 16px;" </span>  The model simulates the presidential 20,000 times. These are the most common electoral results.</span> </strong> <hr>
</div>')
```
...
ggplotly(final_evs.gg,) %>%
  layout(title = 'A Tale of 20,000 Individual Simulations',
...
htmltools::HTML('<div style="font-family:futura; font-size: 30px; width: 900px; float: left;" id="Header">
<strong>State Outcomes<br> <span style="font-size: 16px;" </span> The highlighted bars represents an 80% chance that the margin of victory <br >will lie inside the range. </strong> <hr>
</div>')
```
...
ggplotly(p4) %>%
  layout(margin = list(t=47),title = 'State Outcomes',
         font=list(family='Futura', size=16))

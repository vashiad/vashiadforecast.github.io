...
chancemap <- plot_usmap(data = simulations, values = "TrumpStateWinProb", color = "black",size=0.55) + 
  scale_fill_gradient2(
    low = "blue", mid="white",high = "red", name = "% Chance", midpoint=50,label = scales::comma
...
#Histogram
final_evs.gg <-  ggplot(finalEV,aes(x=BidenEV,
                                    fill=ifelse(BidenEV>=270,'Democratic','Republican'))) +
  geom_vline(xintercept = 270) +
  geom_histogram(binwidth=1) +
  theme_minimal() +
  theme(legend.position = 'top',
        panel.grid.minor = element_blank()) +
  scale_fill_manual(name='Electoral College winner',values=c('Biden'='blue','Trump'='red'))+
...
p3 + labs(x = "Chance of Trump Win", y = "State") +
    ...
    htmltools::HTML(
      sprintf("<div style='font-size:12px;width:200px;float:left'>
            ...
            <div style='width:95%%'>
              ...
              <br/>
              <span style='color:blue;float:left'>%s%%</span>
              <span style='color:red;float:right'>%s%%</span><br clear='all'/>
              <span style='background:blue;width:%s%%;float:left'>&nbsp;</span>
              <span style='background:red;width:%s%%;float:right'>&nbsp;</span>
            </div>
...
factpal <- colorFactor(c("blue", "red"), spatialdata@data$who)
...
leaflet(options=leafletOptions(crs = leafletCRS("L.CRS.Simple"),
  ...
  addPolygons(data=spatialdata, group = 'states',
              ...
              label=~hoverText,
              labelOptions = labelOptions(offset = c(-100,-140),
                                          #direction='bottom',
                                          textOnly = T,
                                          style=list(
...
#Coding the interactive leaflet map
# Dems are blue and Reps are red.
factpal <- colorFactor(c("blue", "red"), spatialdata2@data$who)

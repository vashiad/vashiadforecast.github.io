...
<strong>The 2020 Election promises to be one of the most consequential elections in American history.<br> Who is most likely to take The White House? </strong> <br>
</div>')
```
...
```{r,fig.width = 9.5, fig.asp = .70, fig.align = "left"}
...
leaflet(spatialdata2,options=leafletOptions(crs = crs.laea,
  ...
  setView(lng = -98.5795,lat = 39.8283, zoom = 3.4) %>%
  # addLabelOnlyMarkers(
...
```{r,fig.width = 9.5, fig.asp = .82, fig.align = "left"}
#knitr::include_graphics(rep('/Users/aditya/Desktop/vashiadforecast.github.io/index_files/figure-html/BidenHead.png', 1))
...
htmltools::HTML('<div style="font-family:futura; font-size: 14px; width: 900px; float: left;" id="Header">This is an alternative method to look at the election map.<br>Each state is visualized by the number of electoral votes it has. 
</div>')
...
```{r,fig.width = 10, fig.asp = .42, fig.align = "center"}
...
```{r,fig.width = 10, fig.asp = 2.25, fig.align = "center"}
htmltools::HTML('<div style="font-family:futura; font-size: 30px; width: 900px; float: left;" id="Header">
